...
const MODELS = (Array.isArray(config.models) ? config.models : []).map((m) => {
  ...
  return {
    ...
    owned_by: item.owned_by ?? "fake-ai",
  };
});
...
async function handle(req, res) {
  ...
  // 健康检查（无需 Key，方便部署后确认服务活着）
  if (method === "GET" && (p === "/" || p === "/health")) {
    return sendJson(res, 200, {
      status: "ok",
      name: "fake-ai-api",
      message: "这是一个恶搞用的 Fake AI API，永远只会返回 ascii.txt 的内容。",
  ...
  if (method === "GET" && p === "/v1/models") {
    if (!isAuthorized(req)) {
      return sendOpenAIError(
        ...
        401,
        "invalid_api_key",
        "Incorrect API key provided. You can find the key in the fake-ai-api logs or config."
      );
    }
