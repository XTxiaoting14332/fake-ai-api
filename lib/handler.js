...
// 取最后一条用户消息的真实文本（跳过 system-reminder 注入块）
...
function computeOutput(body, maxField = "max_tokens", stopField = "stop") {
  ...
  // 错误格式的工具结果（tool_use_error / JSON error / Exception）不承接，直接落回问候/婉拒/ascii
  const rawResult = pendingToolResult(body) ? toolResultText(body) : null;
  const looksLikeError =
    rawResult &&
    (rawResult.includes("tool_use_error") ||
      rawResult.includes('"error"') ||
      rawResult.includes("Exception") ||
      rawResult.includes("No such tool"));
  const toolResult = rawResult && !looksLikeError ? rawResult : null;
  let text =
    toolResult !== null
      ? toolResult
